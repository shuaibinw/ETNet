...
from sklearn.metrics import roc_curve, precision_recall_curve  # 移除 auc 导入
from sklearn.metrics import auc as sklearn_auc  # 重命名导入的 auc 函数
import matplotlib
matplotlib.use('Agg')
...
from sklearn.metrics import precision_score, recall_score  # 添加精确率和召回率计算
# 设置GPU
...
def test_model(model_path):
    ...
    roc_auc_value = roc_auc_score(test_labels, predictions)  # 重命名变量
    ...
    roc_auc = sklearn_auc(fpr, tpr)  # 使用重命名的 auc 函数
    
    # 保存 ROC 曲线数据到文件
    with open('Feature2/GM12878_ROC.txt', 'w') as f:
        f.write(f"GM12878 ROC Curve Data\n")
        f.write(f"=====================\n\n")
    ...
    # 保存 PR 曲线数据到文件
    with open('Feature2/GM12878_PRC.txt', 'w') as f:
        f.write(f"GM12878 Precision-Recall Curve Data\n")
        f.write(f"================================\n\n")
    ...
    # 将评估指标保存到文件
    with open('Feature2/GM12878_metrics.txt', 'w') as f:
        f.write(f"GM12878 Model Evaluation Metrics\n")
        f.write(f"================================\n\n")
    ...
    roc_df.to_csv('Feature2/GM12878_ROC.csv', index=False)
    ...
    pr_df.to_csv('Feature2/GM12878_PRC.csv', index=False)
...
if __name__ == "__main__":
    model = Enhancer_MDLF()
    # training(model)
    model_path = 'GM12878_0.87.h5'  # 训练脚本保存的模型路径
...
#     model = Enhancer_MDLF()
#     training(model)
#     model_path = 'GM12878.h5'  # 训练脚本保存的模型路径
#     test_model(model_path)
